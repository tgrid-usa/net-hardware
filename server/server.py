...
# Sample credentials (you can replace these with database or environment variables)
...
async def register(request):
    ...
    # Get the client-id and client-secret from headers
    client_id = request.headers.get('client-id')
    client_secret = request.headers.get('client-secret')

    # Validate the headers
    if client_id is None or client_secret is None:
        return json_response({"error": "missing token... pass token!", "success": False})
    ...
    if client_id == VALID_CLIENT_ID and client_secret == VALID_CLIENT_SECRET:
        ...
        return json_response({"message": "Decentralize Identity Created!","success": True, "seed": seed, "did": did, "verkey": verkey})
    else:
        return json_response({"error": "Invalid token.., Auth Failed", "success": False})
...
async def authapi(request):
    ...
    try:
        ...
        return json_response({"error": "Body is missing or invalid JSON", "success": False})

    if body is None:
        return json_response({"error": "Body is missing", "success": False})

    result = body.get("data")

    # Validate the headers
    if client_id is None or client_secret is None:
        return json_response({"error": "client-id or client-secret header missing", "success": False})
    
    if client_id == VALID_CLIENT_ID and client_secret == VALID_CLIENT_SECRET:
        return json_response({"message": "Client authenticated successfully!", "result": result, "success": True})
    else:
        return json_response({"error": "Invalid client-id or client-secret", "success": False})
